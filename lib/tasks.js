...
const tasks = {
  ...
  inlineSources: (files, src, dest) => cb => inlineSources(files, src, dest, cb),
...
function watch(files, config, cb) {
  ...
  gulp.watch(globs.src.html, tasks.inlineSources(files, config.src, config.dest));
  gulp.watch(globs.src.css, tasks.inlineSources(files, config.src, config.dest));
  cb();
}
